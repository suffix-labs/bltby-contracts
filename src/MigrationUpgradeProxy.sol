...
contract CustomTransparentProxy is ERC1967Proxy {
    ...
    constructor(address implementation, address admin, bytes memory data) payable ERC1967Proxy(implementation, data) {
        _admin = admin;
...
contract MigrationAndUpgradeProxy is Ownable {
     ...
     */
    function deployProxy(address logicContract, bytes memory data, string memory contractName) external onlyOwner {
        ...
        CustomTransparentProxy proxy = new CustomTransparentProxy(logicContract, address(proxyAdmin), data);
     ...
     */
    function getProxyAddress(string memory contractName) external view returns (address) {
     ...
     */
    function getProxyAdmin(string memory contractName) external view returns (address) {
     ...
     */
    function upgradeProxy(string memory contractName, address newImplementation) external onlyOwner {
        ...
        admin.upgradeAndCall(ITransparentUpgradeableProxy(proxyAddress), newImplementation, "");
    }
     ...
     */
    function upgradeProxyAndCall(string memory contractName, address newImplementation, bytes memory data)
        external
        onlyOwner
    {
        address proxyAddress = proxies[contractName];
        ...
        ProxyAdmin admin = ProxyAdmin(adminAddress);
        admin.upgradeAndCall(ITransparentUpgradeableProxy(proxyAddress), newImplementation, data);
    }
}
