...
contract Governance is AccessControl {
    ...
    constructor(address _bltbyToken, address _membershipNFT, address _investorNFT) {
        ...
        _grantRole(FOUNDER_DIRECTOR_ROLE, msg.sender);
    ...
    function _validateProposalCreation(uint8 category) private view {
        ...
        if (bltbyToken.balanceOf(msg.sender) < STAKE_AMOUNT) revert InsufficientStake();
    }
    ...
    function _createNewProposal(uint256 proposalId, string memory description, uint256 duration, uint8 category) private {
    ...
    function _validateVote(uint256 proposalId, uint256 nftId) private view {
        ...
        if (block.timestamp > proposal.endTime) revert VotingPeriodOver(proposalId);
